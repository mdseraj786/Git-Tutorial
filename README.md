...
- merge does not look like linear that's why I used `git rebase`

- `git rebase <branch_name>`  => created a copy of branch commit to current branch
more cleaner way to merge the branches together

`squashing` meaning merging multiple commit into one commit 
note - go to that branch that you want to actually merge

-  `git rebase --interactive main`  => use for interacting mode -- in this mode it is so smart it will  list the all the commit of the branch that it will be merge 
- here we need to have one "pick" and other can be "squash" values
- if you squash using main branch it will not show the other commit --- that's why for squashing use only those branch those you want to merge/rebase to other(main)

- we can also merge/squash using GitHub action like (merge pull request)
